...
export const InteractiveButton = React.forwardRef<HTMLButtonElement, InteractiveButtonProps>(
  ({ className, children, disabled, ...props }, ref) => {
    return (
      <button
        className={cn(
          "group relative flex items-center justify-center overflow-hidden rounded-lg bg-primary px-4 py-2 transition-transform hover:scale-105 active:scale-95",
      ...
      >
        ...
        `}</style>
        <span className="relative z-20 font-medium text-primary-foreground">
          {children}
        </span>
      </button>
    );
